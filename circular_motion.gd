...
# FB[C656a] REJECTED - `extends Control` is imposed by the engine; every Godot
#   script inherits something. To earn C656 you need inheritance you designed -
#   an authored base class. See the grading doc for where that could honestly live.
#
# ============================================================================
...
# FB[C634a] REJECTED - an @onready node reference is not a global variable; it is
#   a handle to a child node of this scene. The project's true globals are in
#   Global.gd - currently unlabelled AND untyped. Typing them is the fix that
#   earns C634 honestly.
...
#   arrays driving a loop) - move the label where the evidence is strongest.
...
# FB[C741a] MISLABEL - the comment text says 'interface controls' (that is
#   C731's wording); C741 is naming conventions for CODE STRUCTURES. And this
#   function name is engine-autogenerated, so it demonstrates neither. If you
#   claim C741, claim it on a name you chose: _try_parse_validated_float.
